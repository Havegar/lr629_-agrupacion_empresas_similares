...
# ### Lectura de datos de familiares, construccion de relaciones familiares

# In[35]:


#Se lee la data,se descartan las entradas erroneas.
# Selecionaremos solo las relaciones de PADRE, MADRE, HIJO, CONYUGE
...
spark.sql('select * from familiares').createOrReplaceTempView("familiares")
...
spark.sql('select CONT_RUT, RUT_FAM, TIPO_RELACION from familiares where ( TIPO_RELACION ="PADRE" OR  TIPO_RELACION ="MADRE" OR  TIPO_RELACION ="HIJO" OR TIPO_RELACION ="CONYUGE")').createOrReplaceTempView("familiares")
...
df_relaciones_familiares=spark.sql('select CONT_RUT,RUT_FAM from familiares').toPandas()


# In[37]:


# Se agregan los datos de las personas naturales fmailiares al pool de entidades
spark.sql('SELECT CONT_RUT as RUT_UNICO, "PERSONA" AS TIPO FROM familiares UNION SELECT RUT_FAM as RUT_UNICO, "PERSONA" AS TIPO FROM familiares UNION SELECT RUT_UNICO, TIPO FROM entidades').createOrReplaceTempView("entidades")
...
# ### Lectura de datos de contadores y definicion de tipos de RUT, construccion de relaciones de contabilidad
...
# cargamos relaciones, con nodos cont_rut y cont_rut_fam
for _, row in df_relaciones_familiares.iterrows():
    G.add_node(row['CONT_RUT'])
    G.add_node(row['RUT_FAM'])
    G.add_edge(row['CONT_RUT'],row['RUT_FAM'])   
...
df_entidades_final.to_csv('/home/cdsw/data/processed/louvain_malla_societaria_contador_representante_familiares/louvain_malla_societaria_contador_representante_familiares.csv', index=False)
# In[ ]:
...
# Guardar el DataFrame de resolución versus modularidad como un archivo CSV en la misma dirección que el otro archivo
df_resolucion_modularidad = pd.DataFrame({'resolucion': resoluciones, 'modularidad': modularidades})
df_resolucion_modularidad.to_csv('/home/cdsw/data/processed/louvain_malla_societaria_contador_representante_familiares/resolucion_modularidad.csv', index=False)
