...
malla_noduplicados=spark.sql('select RUT_SOCIEDAD, RUT_SOCIO from malla_societaria order by RUT_SOCIEDAD ASC LIMIT 10000')
...
### guardamos los datos de todas las entidaddes de la malla societaria, indicando si 
...
spark.sql('SELECT * from libsdf.arfi_contadores_e order by CONT_RUT asc limit 1000').createOrReplaceTempView("data_contador")
...
#spark.sql('SELECT RUT_UNICO, TIPO FROM entidades WHERE RUT_UNICO IN ( SELECT RUT_UNICO FROM entidades GROUP BY RUT_UNICO HAVING COUNT(DISTINCT TIPO) > 1) ORDER BY RUT_UNICO ASC').show()
...
df_entidades_final.to_csv('/home/cdsw/data/processed/louvain_malla_societaria_contador/louvain_malla_societaria_contador.csv', index=False)
...
df_resolucion_modularidad.to_csv('/home/cdsw/data/processed/louvain_malla_societaria_contador/resolucion_modularidad.csv', index=False)
